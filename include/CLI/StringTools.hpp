...
// [CLI12:string_tools_hpp:verbatim]

/// Include the items in this namespace to get free conversion of enums to/from streams.
/// (This is available inside CLI as well, so CLI12 will use this without a using statement).
namespace enums {
...
CLI12_MODULE_INLINE constexpr int expected_max_vector_size{1 << 29};
...
CLI12_INLINE std::ptrdiff_t find_member(std::string name,
...
CLI12_INLINE std::vector<std::string> split_up(std::string str, char delimiter = '\0');

/// get the value of an environmental variable or empty string if empty
CLI12_INLINE std::string get_environment_value(const std::string &env_name);
...
CLI12_INLINE std::string binary_escape_string(const std::string &string_to_escape, bool force = false);
...
CLI12_INLINE bool process_quoted_string(std::string &str,
                                        char string_char = '\"',
                                        char literal_char = '\'',
